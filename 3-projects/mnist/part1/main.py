...
#     print(f'Linear Regression test_error (lambda = {i}) =', run_linear_regression_on_MNIST(lambda_factor=i))
...
# for reg in C:
#     print(f'SVM one vs. rest test_error, C = {reg}:', run_svm_one_vs_rest_on_MNIST(reg))
...
def run_softmax_on_MNIST(temp_parameter=1):
    ...
    train_y_updated, test_y_updated = update_y(train_y, test_y)
    test_error_mod3 = compute_test_error_mod3(test_x, test_y_updated, theta, temp_parameter)
    print(f'Softmax test error mod3 = {test_error_mod3}')
...
#     print(f'Softmax test error (temp = {temp}) = {run_softmax_on_MNIST(temp_parameter=temp)}')
...
# print(f'Softmax test error mod_3 = {run_softmax_on_MNIST_mod3()}')
...
def run_pca18_softmax(temp_parameter=1):
    # The following creates new train_x and test_x by projecting onto principal components
    n_components = 18
    train_x_centered, feature_means = center_data(train_x)
    pcs = principal_components(train_x_centered)
    train_pca = project_onto_PC(train_x, pcs, n_components, feature_means)
    ...
    # The following runs the softmax regression algorithm and reports the test error
...
# print(f"PCA18 softmax test error: {run_pca18_softmax()}")
...
def run_pca10_softmax(temp_parameter=1):
    """
    Runs the softmax algorithm on the cubic kernel PCA10 data.
    Args:
        temp_parameter: temperature parameter.

    Returns:
        test_error: the test error of the PCA10 softmax algorithm.
    """
    # The following runs the softmax regression algorithm and reports the test error
...
# print(f"PCA10 softmax test error: {run_pca10_softmax()}")


def poly_svm():
    clf = SVC(random_state=0, kernel='poly', degree=3, gamma='auto')
    clf.fit(train_pca10, train_y)
    y_pred = clf.predict(test_pca10)

    accuracy = clf.score(test_pca10, test_y)
...
# print(f"Polynomial SVM test error (degree 3) = {poly_svm()}")
print(f"Radial Basis Function kernel SVM test error = {rbf_svm()}")
